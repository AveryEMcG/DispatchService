...
def test_response_ok():
    proc = subprocess.Popen(api_command,
...
def test_email_generated():
    ...
    with open("emails.txt","r") as fp:
        ...
        assert('Dear Voltan' in fp.read()), "test_email_generated: email file was not written"


def test_error_logged():
    proc = subprocess.Popen(api_command_bad,
        stdout = subprocess.PIPE,
        stderr = subprocess.PIPE, shell=True    
        )
    stdout, stderr = proc.communicate()
    ...
    with open("error_logger_log.txt","r") as fp:
            assert(message_types.ERROR_UNSUPPORTED_EVENT in fp.read()), "test_error_logged: invalid event was not logged properly"
