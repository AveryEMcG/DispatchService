...
class DispatchServer(BaseHTTPRequestHandler):
    ...
    def write_response_to_user(self,message,code):
        ...
        self.end_headers()
        self.wfile.write(bytes(message, "utf-8")) #wfile.write is annoying, requires byte format.
        return
    ...
    def do_POST(self):
        ...
        if not self.user_verified(arguments):
            self.error_queue.put(message_types.Error_data_packet(arguments, message_types.ERROR_UNVERIFIED_USER_REQUEST))    
            self.write_response_to_user("You are not verified to do this action\n",400)
            return

        if not self.input_valid(arguments):
            self.error_queue.put(message_types.Error_data_packet(arguments, message_types.ERROR_INVALID_DATA_TO_API))    
            self.write_response_to_user("The data you have provided is invalid. Please check your arguments.\n",400)
            return
